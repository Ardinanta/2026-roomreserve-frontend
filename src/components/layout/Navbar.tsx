...
export default function Navbar() {
    ...
    const linkClass = (path: string) =>
        `px-6 py-2.5 rounded-full text-base font-medium transition-all duration-200 ${
            isActive(path)
        ...
        <nav className="bg-slate-950 sticky top-0 z-50 shadow-lg shadow-black/10">
            <div className="w-full px-8">
                <div className="flex justify-between items-center h-[100px]">
                    ...
                    {/* Logo */}
                    <Link to="/" className="flex items-center gap-3.5 group" onClick={closeMenu}>
                        <div className="w-11 h-11 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:shadow-blue-500/50 transition-all duration-300 group-hover:scale-105">
                    ...
                    {/* Desktop Nav - Center */}
                    <div className="hidden md:flex items-center gap-1 absolute left-1/2 -translate-x-1/2 bg-white/5 backdrop-blur-sm rounded-full px-2 py-1.5 border border-white/5">
                    ...
                    <div className="hidden md:flex items-center gap-4 ml-auto">
                        {isAuthenticated ? (
                            <>
                                <div className="flex items-center gap-3.5 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl px-5 py-2.5 hover:bg-white/10 transition-all duration-200">
                                    <div className="w-10 h-10 bg-gradient-to-br from-emerald-400 to-cyan-500 rounded-xl flex items-center justify-center text-white text-base font-bold shadow-lg shadow-emerald-500/20">
                                        {user?.fullName?.charAt(0).toUpperCase()}
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-base font-semibold text-white leading-tight">{user?.fullName}</span>
                                ...
                                <button onClick={handleLogout}
                                    className="px-6 py-3 bg-red-500/10 hover:bg-red-500 border border-red-500/20 hover:border-red-500 text-red-400 hover:text-white text-base font-semibold rounded-xl transition-all duration-200 cursor-pointer hover:shadow-lg hover:shadow-red-500/25">
                                    Logout
                                </button>
                            </>
                        ) : (
                            <div className="flex items-center gap-3">
                                <Link to="/login" className="text-slate-400 hover:text-white text-base font-medium px-6 py-3 rounded-xl hover:bg-white/5 transition-all" onClick={closeMenu}>
                                    Login
                                </Link>
                                <Link to="/register" className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-base font-semibold px-7 py-3 rounded-xl shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:scale-105 transition-all" onClick={closeMenu}>
                                    Register
                                </Link>
                            </div>
                        )}
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${menuOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
                <div className="px-6 pb-6 pt-2 border-t border-white/5 space-y-2">
                    ...
                    <div className="border-t border-white/5 pt-4 mt-3 space-y-3">
                        {isAuthenticated ? (
                            <>
                                <div className="flex items-center gap-3.5 bg-white/5 rounded-2xl px-5 py-4">
                                    <div className="w-11 h-11 bg-gradient-to-br from-emerald-400 to-cyan-500 rounded-xl flex items-center justify-center text-white text-base font-bold">
                                        {user?.fullName?.charAt(0).toUpperCase()}
                                    </div>
                                    <div className="flex flex-col">
                                ...
                                <button onClick={handleLogout}
                                    className="w-full py-3 bg-red-500/10 hover:bg-red-500 border border-red-500/20 hover:border-red-500 text-red-400 hover:text-white text-base font-semibold rounded-xl transition-all cursor-pointer">
                                    Logout
                                </button>
                            </>
                        ) : (
